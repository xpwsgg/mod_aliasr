小声BB：不知道原版在哪里了，自己用阿里云nls最新的（2024年04月29日）sdk糊了一个。
...
```bash
# 把aliasr.conf.xml参数配置好后复制到/usr/local/freeswitch/conf/autoload_configs/
...
`start_aliasr` 支持可选参数 `read`（识别对端说话）或不传（识别本端说话）：

```
# 识别对端（被叫方）说话
originate user/1001 'start_aliasr read,echo' inline
...
#### 回调事件
在aliasr.conf.xml中有个callback-url，可以配置一个http地址，每次（实时）识别结束后，会把识别结果post到这个地址。
